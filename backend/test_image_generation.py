...
def test_image_service():
    ...
    if not api_key:
        print("    ⚠️ GOOGLE_GEMINI_API_KEY not set; skipping live image generation step")
    else:
    ...
    try:
        from app.services.image_service import ImageService, _normalize_image_mime
        ...
        assert False, f"Failed to initialize ImageService: {e}"
    ...
    try:
        ...
        if endpoint_found:
            ...
            assert False, f"/generate-image endpoint not found. Available routes: {routes}"
    except Exception as e:
        print(f"    ❌ Failed to check endpoints: {e}")
        assert False, f"Failed to check endpoints: {e}"
    
    # Test 5: Test helper behavior without external API call
    print("\n[5] ✓ Testing image helper behavior...")
    try:
        assert _normalize_image_mime("image/jpeg", "") == "image/jpeg"
        assert _normalize_image_mime(None, "https://example.com/file.webp") == "image/webp"
        assert _normalize_image_mime(None, "https://example.com/file.unknown") == "image/png"
        print("    ✅ MIME normalization helper works as expected")
    except Exception as e:
        print(f"    ❌ Failed to validate image helper behavior: {e}")
        assert False, f"Failed to validate image helper behavior: {e}"
    
    # Test 6: Generate actual image (if API key is valid)
    print("\n[6] ✓ Testing async image generation...")
    if api_key:
        try:
            result = asyncio.run(service.generate_image("A test image", size="1024x1024"))
            ...
            print(f"    (Continuing: external provider may be unavailable/quota-limited)")
